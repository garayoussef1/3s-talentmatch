"""Seed (create or update) a dev admin user without relying on email OTP.

Usage (from repo root):
  .\.venv-10\Scripts\python.exe .\backend\scripts\seed_dev_admin.py --email admin@local.test --password "Admin123!"

This script is NON-DESTRUCTIVE: it does not truncate tables.
It simply creates (or updates) a user and marks it as email-verified.
"""
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Seed (create/update) a dev admin user")
    ...
    try:
        ...
        if user:
            ...
            db.commit()
            print(f"✅ Admin mis à jour: email={args.email} role=admin verified=True")
        else:
            user = create_user(
                db,
                nom=args.nom,
            ...
            user.verification_code_expires = None
            db.commit()
            print(f"✅ Admin créé: email={args.email} role=admin verified=True")
